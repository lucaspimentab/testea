...
import { ongs as seedOngs, seedDatabase } from "./seed.tsx";
...
(async () => {
  ...
  if (!adminUser) {
    ...
    await kv.set('user:admin@ong.com', {
      ...
      name: 'Administrador',
      role: 'admin',
      ongId: firstOng?.id || null, // Relacionar com a primeira ONG
      createdAt: new Date().toISOString()
    });
    console.log('✅ Admin user created: admin@ong.com / admin123');
    if (firstOng) {
...
async function verifyAuth(authHeader: string | null) {
  ...
  return { error: null, user: { id: token, email: token, ...user } };
}
...
app.post("/make-server-7a062fc7/debug/recreate-admin", async (c) => {
  try {
    ...
    if (allOngs.length === 0) {
      ...
      return c.json({ error: 'No ONGs found. Please seed database first by clicking "📊 Criar Métricas e Dados".' }, 400);
    }
    ...
    return c.json({ error: 'Error recreating admin: ' + error.message }, 500);
  }
});
...
// Sign up - create new admin user + ONG (all key fields required)
app.post("/make-server-7a062fc7/signup", async (c) => {
  try {
    ...
    if (!ongNome) missing.push('ongNome');
    if (!ongDescricao) missing.push('ongDescricao');
    if (!ongEmail) missing.push('ongEmail');
    if (!telefone) missing.push('telefone');
    if (!endereco) missing.push('endereco');
    if (!bairro) missing.push('bairro');
    if (!regiao) missing.push('regiao');
    if (!horarios || (Array.isArray(horarios) && horarios.length === 0)) missing.push('horarios');
    if (!areasAtuacao || (Array.isArray(areasAtuacao) && areasAtuacao.length === 0)) missing.push('areasAtuacao');
    if (!idiomas || (Array.isArray(idiomas) && idiomas.length === 0)) missing.push('idiomas');
    ...
    const ongRecord = {
      ...
      regiao,
      site: site || '',
      horarios: normalizeList(horarios),
      areasAtuacao: normalizeList(areasAtuacao),
      idiomas: normalizeList(idiomas),
      verificada: false,
      imagens: [],
      logo: '',
    ...
    await kv.set(`ongs:${ongId}`, ongRecord);

    // Initialize metrics for the new ONG
    await kv.set(`metricas:${ongId}`, {
      ongId,
      visualizacoes: 0,
      cliques: 0,
      interessados: 0,
    ...
    // Create new user already linked to the ONG
    const newUser = {
      email,
    ...
    return c.json({
      ...
      ong: ongRecord,
      token: email, // simple token for this prototype
    });
  } catch (error: any) {
...
app.get("/make-server-7a062fc7/ongs", async (c) => {
  try {
    ...
    if (regiao) {
      filtered = filtered.filter((ong: any) => ong.regiao === regiao);
    }
    if (bairro) {
      filtered = filtered.filter((ong: any) => ong.bairro === bairro);
    }
    if (areaAtuacao) {
      filtered = filtered.filter((ong: any) => ong.areasAtuacao?.includes(areaAtuacao));
    }
    if (idioma) {
      filtered = filtered.filter((ong: any) => ong.idiomas?.includes(idioma));
    }
    if (horario) {
      filtered = filtered.filter((ong: any) => ong.horarios?.includes(horario));
    }
    if (busca) {
      const searchTerm = busca.toLowerCase();
      filtered = filtered.filter((ong: any) => 
        ong.nome?.toLowerCase().includes(searchTerm) ||
        ong.descricao?.toLowerCase().includes(searchTerm) ||
        ong.areasAtuacao?.some((area: string) => area.toLowerCase().includes(searchTerm))
      );
    }

    return c.json({ ongs: filtered });
  } catch (error) {
    console.error('Error fetching ONGs:', error);
    return c.json({ error: 'Error fetching ONGs' }, 500);
  }
});
...
app.get("/make-server-7a062fc7/ongs/:id", async (c) => {
  try {
    ...
    return c.json({ error: 'Error fetching ONG' }, 500);
  }
});
...
app.post("/make-server-7a062fc7/ongs", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error creating ONG' }, 500);
  }
});
...
app.put("/make-server-7a062fc7/ongs/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ ong: updatedOng });
  } catch (error) {
    console.error('❌ Error updating ONG:', error);
    return c.json({ error: 'Error updating ONG', details: error.message }, 500);
  }
});
...
app.delete("/make-server-7a062fc7/ongs/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error deleting ONG' }, 500);
  }
});
...
app.get("/make-server-7a062fc7/vagas", async (c) => {
  try {
    ...
    let filtered = vagas;
    if (ongId) {
      filtered = filtered.filter((vaga: any) => vaga.ongId === ongId);
    }

    return c.json({ vagas: filtered });
  } catch (error) {
    console.error('Error fetching vagas:', error);
    return c.json({ error: 'Error fetching vagas' }, 500);
  }
});
...
app.get("/make-server-7a062fc7/vagas/:id", async (c) => {
  try {
    ...
    if (!vaga) {
      return c.json({ error: 'Vaga not found' }, 404);
    }

    return c.json({ vaga });
  } catch (error) {
    console.error('Error fetching vaga:', error);
    return c.json({ error: 'Error fetching vaga' }, 500);
  }
});
...
app.post("/make-server-7a062fc7/vagas", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error creating vaga' }, 500);
  }
});
...
app.put("/make-server-7a062fc7/vagas/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ vaga: updatedVaga });
  } catch (error) {
    console.error('Error updating vaga:', error);
    return c.json({ error: 'Error updating vaga' }, 500);
  }
});
...
app.delete("/make-server-7a062fc7/vagas/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error deleting vaga' }, 500);
  }
});
...
app.post("/make-server-7a062fc7/perguntas", async (c) => {
  try {
    ...
    return c.json({ pergunta });
  } catch (error) {
    console.error('Error creating pergunta:', error);
    return c.json({ error: 'Error creating pergunta' }, 500);
  }
});
...
app.put("/make-server-7a062fc7/perguntas/:id", async (c) => {
  ...
  try {
    ...
    if (!existingPergunta) {
      return c.json({ error: 'Pergunta not found' }, 404);
    }
    ...
    return c.json({ pergunta: updatedPergunta });
  } catch (error) {
    console.error('Error answering pergunta:', error);
    return c.json({ error: 'Error answering pergunta' }, 500);
  }
});
...
app.delete("/make-server-7a062fc7/perguntas/:id", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error deleting pergunta' }, 500);
  }
});
...
app.get("/make-server-7a062fc7/metricas/:ongId", async (c) => {
  ...
  try {
    ...
    return c.json({ error: 'Error fetching metricas' }, 500);
  }
});
...
app.post("/make-server-7a062fc7/metricas/:ongId", async (c) => {
  try {
    ...
    return c.json({ error: 'Error updating metricas' }, 500);
  }
});
...
app.get("/make-server-7a062fc7/documentos/:ongId", async (c) => {
  ...
  try {
    ...
    const filtered = documentos.filter((doc: any) => doc.ongId === ongId);

    return c.json({ documentos: filtered });
  } catch (error) {
    console.error('Error fetching documentos:', error);
    return c.json({ error: 'Error fetching documentos' }, 500);
  }
});
