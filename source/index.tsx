...
app.post("/make-server-7a062fc7/debug/recreate-admin", async (c)=>{
  try {
    ...
    // Preferir a ONG custom (id fixo ou nome) para conta admin
...
app.post("/make-server-7a062fc7/signup", async (c)=>{
  try {
    ...
    // Seed métricas e pergunta inicial para essa ONG
    await kv.set(`metricas:${ongId}`, {
      ...
      interessados: 7,
      historico: [
        { data: new Date().toISOString(), acao: 'view' },
        { data: new Date().toISOString(), acao: 'click' }
      ]
    });
    const perguntaId = crypto.randomUUID();
    await kv.set(`perguntas:${perguntaId}`, {
      id: perguntaId,
      ongId,
      nome: 'Visitante',
      email: 'visitante@example.com',
      mensagem: 'Gostaria de saber mais sobre as atividades.',
      respondida: false,
